...
- `MACOS_PLUGIN_TARGETS`: optional module targets installed under the sibling `plugins` tree. A target may provide `DEPLOYKIT_MACOS_PLUGIN_DESTINATION`, `DEPLOYKIT_MACOS_PLUGIN_RUNTIME_DIRECTORIES`, `DEPLOYKIT_MACOS_PLUGIN_RUNTIME_PAYLOAD`, and, when package metadata is needed, both `DEPLOYKIT_MACOS_PLUGIN_MANIFEST` and `DEPLOYKIT_MACOS_PLUGIN_ICON`; DeployKit installs and signs the complete package tree.
...
- macOS: installs an ad-hoc signed standalone app bundle plus a sibling `plugins/<id>/current` tree, runs `macdeployqt`, keeps Core libraries in `Contents/Frameworks`, skips copying package-owned Mach-O files out of plugin `runtime`, recursively scans dependencies, and signs app and plugin binaries. The generic CPack path still stages a DragNDrop app-only DMG; a consumer that distributes the sibling tree must use its product installer entrypoint. Set `DEPLOYKIT_MACOS_ADHOC_SIGNING=OFF` only for unsigned diagnostics; production distribution still needs appropriate Developer ID signing and notarization.
